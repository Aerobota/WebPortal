/**
 * Created by u.di.staso on 23/09/14.
 */
/*global define*/
define(['../Core/createGuid',
    ...
    function readAVGLatLonCoordinate(element) {
        //TODO: height is referenced to altitude mode

        var line = element.replace('\n','');
        var tuples = line.trim().split(/[\s\n]+/g);
        var numberOfCoordinates = tuples.length;
        ...
        result[0] = x/numberOfCoordinates;
        result[1] = y/numberOfCoordinates;

        return result;
    }

    function readLatLonCoordinate(element) {
        //TODO: height is referenced to altitude mode
    ...
    function processPolygon(dataSource, entity, kml, node) {
        ...
        if (defined(extrude) && extrude) {
            ...
            /* MIO CUSTOM */
            if(defined(globalHeights[entity.name].height)){
                entity.polygon.extrudedHeight = new ConstantProperty(globalHeights[entity.name].height);
                entity.polygon.height = new  ConstantProperty(globalHeights[entity.name].height);
            }
            else{
    ...
    function loadKml(dataSource, kml, sourceUri, uriResolver) {
        ...
        if (document.length > 0) {
            ...
            var tempHeights = [];
            var coordinates = kml.getElementsByTagName('coordinates');
            var names = kml.getElementsByTagName('Placemark');

            for(var i=0;i<names.length;i++){
                var buildingname = names[i].getElementsByTagNameNS('*',"name")[0].childNodes[0].nodeValue;
                var buildinpositions = names[i].getElementsByTagNameNS('*',"coordinates")[0].textContent;

                /*if(names[i].children[3].nodeName === 'Polygon')
                    var buildinpositions = names[i].children[3].children[3].textContent;
                else
                    var buildinpositions = names[i].children[3].children[3].children[3].textContent;*/
                var AVG = readAVGLatLonCoordinate(buildinpositions);

                tempHeights[i] = Cartographic.fromDegrees(AVG[0], AVG[1]);
                globalHeights[buildingname] = i;
